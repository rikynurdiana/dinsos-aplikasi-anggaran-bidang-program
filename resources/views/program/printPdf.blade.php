...
<html lang="en" dir="ltr">
    <head>
        <meta charset="utf-8">
        <title>PROGRAM DINAS SOSIAL</title>
    ...
    <body>
        ...
        <div class="text-center" style="font-size: 14px;font-weight: bold; text-transform:uppercase;">
            @if (session('user.bidang') == 'admin')
                PROGRAM DINAS SOSIAL TAHUN {{ Carbon\Carbon::now()->format('Y') }}
            @else
                PROGRAM BIDANG {{ session('user.bidang') }} DINAS SOSIAL TAHUN {{ Carbon\Carbon::now()->format('Y') }}
            @endif
        </div>
        ...
        <table class="table table-bordered table-hover" id="myTable" style="font-size:9px">
            ...
            <tbody>
                ...
                @empty
                    <tr>
                        <td colspan="3" class="text-center bold">Tidak Ada Data Ditemukan</td>
                    </tr>
                @endforelse
